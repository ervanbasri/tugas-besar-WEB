...
<html>
    ...
    <body>
        ...
        <div class="hero-unit">
            <?php
            $select = "SELECT * FROM pelanggan WHERE noktp=$noktp";
                ...
                <form method="post" >
                    <table table class="table-hover">
                        <tr>
                            <td>ID USER</td>
                            <td>:</td>
                            <td> <input type="text" name="id" disabled value="<?php echo $row['noktp']; ?>"/> </td>
                        </tr>
                        <tr>
                            <td colspan=3 align="center">
                                <button class="btn btn-large btn-info" type="submit" name="submit">SUBMIT QUERY</button>
                                <button class="btn btn-large btn-danger" type="button" onclick="window.location = 'validasi.php'">CANCEL</button>
                            </td>
                        </tr>
                    </table>
                </form>
            ...
            if (isset($_POST['submit'])) {
                $query = "DELETE FROM pelanggan WHERE noktp=$noktp";
